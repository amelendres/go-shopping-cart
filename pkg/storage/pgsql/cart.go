...
func (r *CartRepository) Get(ID string) (*cart.Cart, error) {
	...
	for rows.Next() {
		p := ProductDTO{}
		err = rows.Scan(&p.ID, &p.Name, &p.Price, &p.Qty)
		if err != nil {
			//break
			return nil, err
		}
		ca.AddProduct(cart.UUID(p.ID), p.Name, cart.Price(p.Price), cart.Quantity(p.Qty))
	}
...
func (q *Queries) insertProduct(cartID string, p cart.Product) error {
	_, err := q.db.Exec(insertProductSql, p.ID(), cartID, p.Name(), p.Price(), p.Qty())
	return err
}
